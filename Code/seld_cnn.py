...
def main(args):
    ...
    data_gen_train = cls_data_generator_seld.DataGenerator(
    ...
    model = seld_model.get_seld_model(data_in=data_in, data_out=data_out, dropout_rate=params['dropout_rate'],
    ...
    for epoch_cnt in range(initial, params['nb_epochs']):
        ...
        print("pred:",pred[0].shape,pred[1].shape)
        if params['mode'] == 'regr':
