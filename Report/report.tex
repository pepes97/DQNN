...
\begin{document}
    ...
    We did it by training the network over about 150 epochs on the whole dataset (not training on splits separately) and by collecting all relevant data.\\
    We trained the network on the whole dataset with the aim to obtain better performace with respect to train the network on splits separately because they include just a subsequent of classes, while we trained our network on the whole audio files (therefore on all the classes simultaneously).\\
    \\
