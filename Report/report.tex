...
\begin{document}
    ...
    A joint SED score can be considered as S$_{SED}$ = $\dfrac{(ER+(1-F))}{2}$.\\
    ...
    A joint DOA score can be defined as S$_{DOA}$ = $\dfrac{(DOA_{err}/180 + (1 - K))}{2}$.\\
    The lower the SDOA score, the better the results in terms of 3D localization.
    Finally, an overall SELD score can be defined based on the previous metrics as S$_{SELD}$ = $\dfrac{(S_{SED} + S_{DOA})}{2}$.\\
    \section{Experiments}
