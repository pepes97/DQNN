...
\begin{document}
	...
	\\ \\ For training our QCNN, we used the winning configuration of our first experiment (with 32 filters in the Quaternion Convolutional layers).
	...
	\\ \\ We trained the models for $\sim$150 epochs.
    \section{Results}
    1) GRAFICO 32 OV1 E OV2 (SELD SCORE, SED SCORE E DOA SCORE)
    \paragraph{}
    \begin{center}
	...
	\paragraph{}
	2) GRAFICO TUTTI CON OV1 (SELD SCORE, SED SCORE E DOA SCORE)
	\\ 3) GRAFICO TUTTI CON OV2 (SELD SCORE, SED SCORE E DOA SCORE)\\
\FloatBarrier

\begin{figure}
\centering

\includegraphics[height=0.4\textwidth]{images/sed_score.png} \\
\caption{$S_{SED}$ evolution computed on the test set during the training}
\vspace{1em}
\includegraphics[height=0.4\textwidth]{images/doa_score.png} \\
\caption{$S_{DOA}$ evolution computed on the test set during the training}
\vspace{1em}
\includegraphics[height=0.4\textwidth]{images/seld_score.png}\\
...
\end{tabular} \\
    ...
    \end{tabular}
\end{center}	
RISULTATI INTERVALLO DI CONFIDENZA (una riga)
\FloatBarrier
\begin{figure}
\centering
